<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Invoice Details') }} #{{ $transaction->id }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="text-left md:text-right">
                        ...
                        <p class="text-gray-600">Invoice #{{ $transaction->id }}</p>
                        <p class="text-gray-600">Date: {{ $transaction->created_at->format('M d, Y') }}</p>
                        <p class="text-gray-600">Status: <span class="font-bold text-green-600">{{ ucfirst($transaction->status) }}</span></p>
                        <p class="text-gray-600">Payment Method: {{ $transaction->payment_method ?? 'N/A' }}</p>
                    </div>
                </div>

                <div class="mt-8">
                    <h2 class="text-lg font-semibold text-gray-700 mb-4">Transaction Summary:</h2>
